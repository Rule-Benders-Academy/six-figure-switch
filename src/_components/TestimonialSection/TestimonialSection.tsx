...
const TestimonialSection = ({ reverseOrder, name, description, videoUrl }: Props) => {
  return (
    <section className="bg-gradient-to-b from-[#141314] to-[#272526] text-white pt-6 pb-10 md:py-14 lg:py-20 px-4">
      <div className="sm:px-8 md:px-16 lg:px-48 flex flex-col lg:flex-row items-center gap-5 md:gap-8 lg:gap-10 w-[90%] lg:w-[90%] mx-auto ">
        <div
          className={`flex-[0.7] text-center lg:text-left ${
            reverseOrder ? "md:order-2" : "order-1"
          }`}
        >
          <h3 className="text-2xl md:text-5xl lg:text-[px] lg:leading-[100%]">
            Real Stories,
          ...
          <h4 className="font-bold text-base md:text-3xl lg:text-4xl mb-1">
            {name.toUpperCase()}
          </h4>
          <p className="text-base md:text-lg lg:text-2xl md:mt-4 font-light">
            {description}
          </p>
          <GradientButton className="!text-sm md:!text-xl lg:!text-[24px] !py-[14px] !px-5 md:!px-8 lg:!px-[35px] max-w-[503px] hidden md:block">
            HELP ME LIKE YOU HELPED {name.toUpperCase()}
          </GradientButton>
        </div>

        <div
          className={`flex-1 w-full -mb-5 md:mb-0 ${
            reverseOrder ? "md:order-1" : "order-2"
          }`}
        >
          {videoUrl ? (
        ...
        <GradientButton
          className={`!text-sm md:!text-xl lg:!text-[24px] !py-[8px] md:!py-[14px] !px-5 md:!px-8 lg:!px-[35px] max-w-[200px] md:max-w-[403px] md:hidden mt-0 ${
            reverseOrder ? "md:order-1" : "order-3"
          }`}
        >
          HELP ME LIKE YOU HELPED {name.toUpperCase()}
        </GradientButton>
